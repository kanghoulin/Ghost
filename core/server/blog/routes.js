...
frontendRoutes = function frontendRoutes() {
    ...
    // ### Extend routes
    /*router.get(/^\/sign-in\/$/, function redirectToSignin(req, res) {
		console.log(utils.url.urlJoin(subdir, '/extend/sign-in/'));
        utils.redirect301(res, utils.url.urlJoin(subdir, '/extend/sign-in/'));
    });
    router.get(/^\/sign-up\/$/, function redirectToSignup(req, res) {
        utils.redirect301(res, utils.url.urlJoin(subdir, '/extend/sign-up/'));
    });
	router.get(/^\/sign-out\/$/, function redirectToSignout(req, res) {
        utils.redirect301(res, utils.url.urlJoin(subdir, '/extend/sign-out/'));
    });*/
